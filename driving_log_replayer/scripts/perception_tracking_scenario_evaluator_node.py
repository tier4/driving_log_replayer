...
"""run tracking evaluation node

- Objective:
    - Evaluate tracking performance of perception algorithms
- Basic assumptions:
    - scenario simulator publishes ground truth and perception results in TrackedObjects topic
...
def tracked_obj_to_dynamic_obj(tracked_obj: TrackedObjects) -> List[DynamicObject]:
    """tracked object to dynamic object

    Args:
        tracked_obj (TrackedObjects): input tracked objects msg

    Returns:
        List[DynamicObject]: output dynamic object list
    """
...
def tracked_obj_to_frame_ground_truth(tracked_obj: TrackedObjects, frame_name: str) -> FrameGroundTruth:
    """tracked object to frame ground truth

    Args:
        tracked_obj (TrackedObjects): input tracked objects msg
        frame_name (str): ground truth frame name. usually str number
    Returns:
        FrameGroundTruth: output frame ground truth
    """
    unix_time: int = eval_conversions.unix_time_from_ros_msg(tracked_obj.header)
    dynamic_objects: List[DynamicObject] = tracked_obj_to_dynamic_obj(tracked_obj)
    # we assume tracked object is only on the "map" frame, so currently we not use ego2map
    ego2map = None
...
class TrackingEvaluator(Node):
    def __init__(self):
        ...
        self.declare_parameter("scenario_path", "/home/yoshiri/autoware/src/simulator/driving_log_replayer/sample/perception/scenario.yaml")
        self.declare_parameter("result_json_path", "/home/yoshiri/autoware/scenario_result.json")
        self.declare_parameter("t4_dataset_path", "")
        self.declare_parameter("result_archive_path", "/home/yoshiri/temp")
        ...
        try:
            ...
            # subscribe both ground truth and perception with message filter
            ...
            #     1,
            # )
            self.gt_sub = Subscriber(self, msg_type, "/simulation/dummy_perception_publisher/debug/ground_truth_objects")
            self.obj_sub = Subscriber(self, msg_type, "/perception/object_recognition/" + evaluation_task + "/objects")
            self.sync_sub = TimeSynchronizer([self.gt_sub, self.obj_sub], 10)
            self.sync_sub.registerCallback(self.perception_cb)
    ...
    def perception_cb(self, gt_msg: TrackedObjects, msg: TrackedObjects):
        ...
        frame_name = len(self.__evaluator.frame_results)
        ground_truth_now_frame = tracked_obj_to_frame_ground_truth(gt_msg, str(frame_name))
        if ground_truth_now_frame is None:
            self.__skip_counter += 1
        else:
            estimated_objects: List[DynamicObject] = tracked_obj_to_dynamic_obj(msg)
...
def main(args=None):
    ...
    perception_evaluator = TrackingEvaluator()
