...
def test_scenario() -> None:
    ...
    assert scenario.Evaluation.Conditions.Threshold["lateral_deviation"] == {
        "min": 10.0,
        "max": 10.0,
        "mean": 10.0,
    }
    assert scenario.Evaluation.Conditions.PassRange == (0.5, 1.05)


def test_range_validation_upper_limit() -> None:
    with pytest.raises(ValidationError):
        Conditions(Threshold={}, PassRange=("0.5-0.95"))


def test_range_validation_lower_limit() -> None:
    with pytest.raises(ValidationError):
        Conditions(Threshold={}, PassRange=("1.05-1.5"))
...
def test_set_threshold_from_file() -> None:
    ...
    threshold = scenario.Evaluation.Conditions.Threshold
...
def test_set_pass_range() -> None:
    ...
    assert scenario.Evaluation.Conditions.PassRange == (0.4, 1.1)
