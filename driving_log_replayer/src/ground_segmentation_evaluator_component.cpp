...
: rclcpp::Node("ground_segmentation_evaluator_component", options)
{
  ...
  non_ground_cloud_sub_.subscribe(
    this, "/perception/obstacle_segmentation/single_frame/pointcloud", rmw_qos_profile_sensor_data);
  ...
  const PointCloud2::ConstSharedPtr eval_target_cloud)
{
  ...
  auto [accuracy, precision, recall, specificity, f1_score] = compute_metrics(tp, fp, tn, fn);
  ...
  eval_result_msg.accuracy = accuracy;
...
std::tuple<float, float, float, float, float> GroundSegmentationEvaluatorComponent::compute_metrics(
  const float TP, const float FP, const float TN, const float FN)
{
  float accuracy = static_cast<float>(TP + TN) / static_cast<float>(TP + FP + TN + FN);
